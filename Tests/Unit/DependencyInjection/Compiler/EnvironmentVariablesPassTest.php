...
class EnvironmentVariablesPassTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function dataProcess()
    {
        ...
        $variables = ['ongr__ongr_settings__environment_variables_pass_test_param' => 'test'];
        $expectedParameter = ['ongr_settings.environment_variables_pass_test_param' => 'test'];
        $out[] = [$variables, $expectedParameter];

        // Case #1 TEST shouldn't be matched.
        $variables = ['TEST__ongr_settings__environment_variables_pass_test_param' => 'test'];
        $expectedParameter = [];
        $out[] = [$variables, $expectedParameter];

        // Case #2 One underscore shouldn't be sufficient.
        $variables = ['ongr_ongr_settings__environment_variables_pass_test_param' => 'test'];
        $expectedParameter = [];
        $out[] = [$variables, $expectedParameter];
